...
struct timer_list freq_mode_timer_elecactive;
...
{
	if (timeout > 0)
		mod_timer(&freq_mode_timer_elecactive, jiffies + msecs_to_jiffies(timeout));
	tbl_select[0] = 2;
	tbl_select[1] = 3;
	tbl_select[2] = 4;
	tbl_select[3] = 4;
...
static void dbs_init_freq_map_table(struct cpufreq_policy *policy)
{
	...
	init_timer(&freq_mode_timer_elecactive);
	freq_mode_timer_elecactive.function = switch_mode_timer;
	freq_mode_timer_elecactive.data = 0;
...
static void dbs_deinit_freq_map_table(void)
{
	...
	del_timer(&freq_mode_timer_elecactive);
}
...
int input_event_boosted_elecactive(void)
{
	unsigned long flags;
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (input_event_boosted_elecactive())
	{
		return;
	}
...
static int dbs_input_connect_elecactive(struct input_handler *handler,
...
static struct input_handler dbs_input_handler = {
	...
	.connect	= dbs_input_connect_elecactive,
...
void set_input_event_min_freq_by_cpu_elecactive ( int cpu_nr, int cpufreq){
